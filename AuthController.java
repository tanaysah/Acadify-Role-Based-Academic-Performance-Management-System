...
public class AuthController {
    ...
    private static void handleLogin(HttpExchange exchange) throws IOException {
        ...
        if (email == null || email.isBlank() || password == null || password.isBlank()) {
            ResponseUtil.sendBadRequest(exchange, "Email and password are required");
            return;
        }
        ...
        try {
            ...
            ResponseUtil.sendServerError(exchange, "Authentication failed");
        } finally {
    ...
    private static void handleRegister(HttpExchange exchange) throws IOException {
        ...
        if (email == null || email.isBlank() || password == null || password.isBlank()
            ...
            ResponseUtil.sendBadRequest(exchange, "Email, password, role, and name are required");
            return;
        }

        role = role.toUpperCase();
        if (!role.equals("STUDENT") && !role.equals("TEACHER") && !role.equals("ADMIN")) {
            ResponseUtil.sendBadRequest(exchange, "Invalid role. Must be STUDENT, TEACHER, or ADMIN");
            return;
        }
        ...
        try {
            ...
            if (!keys.next()) {
                conn.rollback();
                ResponseUtil.sendServerError(exchange, "Registration failed");
                return;
            }
            ...
            ResponseUtil.sendCreated(exchange, "Registration successful", data);
        } catch (SQLException e) {
            if (conn != null) {
    ...
    // Helper method for password hashing (referenced in AdminController)
    static String hashPassword(String password) {
        return PBKDF2Util.hashPassword(password);
    }
}
